...
(function () {
  ...
  function drawCoverCanvas(cover) {
    ...
    ctx.strokeStyle = '#111111';
    ctx.lineWidth = 3;
    ctx.strokeRect(48, 42, 1144, 1646);

    drawPdfText(ctx, '工　事　写　真　帳', 620, 135, 30, {
      align: 'center',
    ...
    const rows = [
      ['現　場　名', cover.siteName || ''],
      ['工　事　名', cover.workName || ''],
      ['工　　期　着　工', cover.startDate || ''],
      ['　　　　　竣　工', cover.finishDate || '']
    ];
    let y = 330;
    rows.forEach(([label, value]) => {
      drawPdfText(ctx, label, 120, y, 22, { font: 'Yu Mincho, Hiragino Mincho ProN, serif' });
      // 入力値は罫線内の中央に表示する
      drawPdfText(ctx, value, 680, y, 23, { align: 'center', font: 'Yu Mincho, Hiragino Mincho ProN, serif' });
      ctx.beginPath();
      ctx.moveTo(260, y + 14);
      ctx.lineTo(1100, y + 14);
      ctx.stroke();
      y += 70;
    });
    if ((cover.showContractor || 'yes') !== 'no') {
      y += 40;
      drawPdfText(ctx, '施　工　者', 120, y, 22, { font: 'Yu Mincho, Hiragino Mincho ProN, serif' });
      // 施工者名も罫線内の中央に表示する
      drawPdfText(ctx, cover.contractorName || '', 665, y, 23, { align: 'center', font: 'Yu Mincho, Hiragino Mincho ProN, serif' });
      ctx.beginPath();
      ctx.moveTo(230, y + 14);
      ctx.lineTo(1100, y + 14);
      ctx.stroke();
  ...
  async function makePhotoPageCanvas(pageEntries) {
    const { canvas, ctx } = makeA4Canvas();
    const photoX = 95, photoW = 650, photoH = 335;
    const infoX = 815, infoW = 290, infoH = 335;
    const topYs = [100, 555, 1010];
    ctx.strokeStyle = '#111111';
    ctx.fillStyle = '#111827';
    for (let idx = 0; idx < pageEntries.length; idx++) {
      ...
      const noH = 62, workH = 84;
      ctx.beginPath();
      ctx.moveTo(infoX, y + noH);
      ctx.lineTo(infoX + infoW, y + noH);
      ctx.moveTo(infoX, y + noH + workH);
      ctx.lineTo(infoX + infoW, y + noH + workH);
      ctx.stroke();
      // 右側情報欄は中央寄せで統一する
      drawPdfText(ctx, 'No.　' + (e.no || ''), infoX + infoW / 2, y + 40, 24, { align: 'center' });
      drawPdfWrappedText(ctx, '工種　' + (e.work_type || ''), infoX + infoW / 2, y + noH + 50, infoW - 24, 21, 18, { align: 'center' });
      const notes = Array.isArray(e.notes) ? e.notes : [];
      const noteTop = y + noH + workH;
      const noteH = (infoH - noH - workH) / NOTE_LINES;
      for (let i = 0; i < NOTE_LINES; i++) {
        const lineY = noteTop + noteH * (i + 1);
        if (i < NOTE_LINES - 1) drawDottedLine(ctx, infoX, lineY, infoX + infoW);
        drawPdfWrappedText(ctx, notes[i] || '', infoX + infoW / 2, noteTop + noteH * i + noteH / 2, infoW - 24, 20, 18, { align: 'center' });
      }
    }
    return canvas;
  }
